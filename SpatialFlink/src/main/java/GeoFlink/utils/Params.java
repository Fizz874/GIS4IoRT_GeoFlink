...
public class Params {
    ...
    //public final String YAML_CONFIG = "rangePointPoint-conf.yml";
    //public final String YAML_CONFIG = "rangePointPolygon-conf.yml";
    //public final String YAML_CONFIG = "speed-conf.yml";
    //public final String YAML_CONFIG = "joinPointPoint-conf.yml";
    public final String YAML_CONFIG = "query1-conf.yml";

//    public final String YAML_PATH = new File(".").getAbsoluteFile().getParent().toString() + File.separator +
//                                    "conf" +  File.separator + YAML_CONFIG;

    public final String YAML_PATH = YAML_CONFIG;
    ...
    public Params(String path) throws NullPointerException, IllegalArgumentException, NumberFormatException {
        ConfigType config = getYamlConfigTarget(path);
    ...
    private ConfigType getYamlConfigTarget(String path) {
        Yaml yaml = new Yaml();

        try {
            InputStream in;

            // 1. Spróbuj wczytać z classpath (resources w JAR)
            in = Params.class.getClassLoader().getResourceAsStream(path);

            // 2. Jeśli nie znaleziono w JAR, spróbuj z dysku
            if (in == null) {
                File file = new File(path);
            ...
            throw new IllegalArgumentException("Could not load YAML config: " + path, e);
        }
    }
