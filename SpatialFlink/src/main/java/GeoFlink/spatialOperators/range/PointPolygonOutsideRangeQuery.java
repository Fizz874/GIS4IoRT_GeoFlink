...
public class PointPolygonOutsideRangeQuery extends RangeQuery<Point, Polygon> {
    public PointPolygonOutsideRangeQuery(QueryConfiguration conf, SpatialIndex index) {
        ...
        if (this.getQueryConfiguration().getQueryType() == QueryType.RealTime) {
            ...
            DataStream<Point> filteredPoints = pointStream.filter(new FilterFunction<Point>() {
                ...
                public boolean filter(Point point) throws Exception {
                    ...
                    return (!(guaranteedNeighboringCells.contains(point.gridID)));
                }
            }).startNewChain();
            ...
            return filteredPoints.keyBy(new KeySelector<Point, String>() {
                ...
                public void flatMap(Point point, Collector<Point> collector) throws Exception {
                    ...
                    if (!candidateNeighboringCells.contains(point.gridID)) {
...
//                            System.out.println("🔹 Poligon: " + polygon.toString());
    ...
    //Experimental method
    public DataSet<Point> runBatch(DataSet<Point> points, Set<Polygon> queryPolygonSet, double queryRadius) {
        ...
        DataSet<Point> filtered = points.filter(point -> !guaranteedNeighboringCells.contains(point.gridID));


        DataSet<Point> result = filtered.flatMap((Point point, Collector<Point> out) -> {
            ...
            if (!candidateNeighboringCells.contains(point.gridID)) {
                ...
                for (Polygon polygon : queryPolygonSet) {
                    double distance;
                    if (this.getQueryConfiguration().isApproximateQuery()) {
                        distance = DistanceFunctions.getPointPolygonBBoxMinEuclideanDistance(point, polygon);
                    } else {
                        distance = DistanceFunctions.getDistance(point, polygon);
                    }
