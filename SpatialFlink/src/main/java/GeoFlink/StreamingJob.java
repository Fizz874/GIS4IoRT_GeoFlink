...
public class StreamingJob implements Serializable {
	...
	public static void main(String[] args) throws Exception {
		...
		Params params = new Params("geoflink-conf.yml");
		...
		/* Input Stream2 */
		String inputTopicName2 = params.inputTopicName2;
		String inputFormat2 = params.inputFormat2;
		String dateFormatStr2 = params.dateFormatStr2;
		List<String> geoJSONSchemaAttr2 = params.geoJSONSchemaAttr2;
		List<String> ordinaryStreamAttributeNames2 = params.geoJSONSchemaAttr2; // default order of attributes: objectID, timestamp
		...
		double cellLengthMeters2 = params.cellLength2;
...
//			queryDateFormat = new SimpleDateFormat(queryDateFormatStr);
		...
		switch(queryOption) {
			...
			case 2: { // Range Query (Real-time) - Point-Stream-Point-Query
				DataStream geoJSONStream  = env.addSource(new FlinkKafkaConsumer<>(inputTopicName, new SimpleStringSchema()/*JSONKeyValueDeserializationSchema(false)*/, kafkaProperties).setStartFromLatest());
				//geoJSONStream.print();
				// Converting GeoJSON,CSV stream to point spatial data stream
				DataStream<Point> spatialPointStream = Deserialization.TrajectoryStream(geoJSONStream, inputFormat, inputDateFormat, inputDelimiter1, csvTsvSchemaAttr1, "timestamp", "oID", uGrid);
			...
			case 3: { //Spatial join query (real time)
				//First stream
				DataStream geoJSONStream1  = env.addSource(new FlinkKafkaConsumer<>(inputTopicName, new SimpleStringSchema(), kafkaProperties).setStartFromLatest());
				DataStream<Point> spatialPointStream1 = Deserialization.TrajectoryStream(geoJSONStream1, inputFormat, inputDateFormat, inputDelimiter1, csvTsvSchemaAttr1, "timestamp", "oID", uGrid);

				//Second stream
				DataStream geoJSONStream2  = env.addSource(new FlinkKafkaConsumer<>(inputTopicName2, new SimpleStringSchema(), kafkaProperties).setStartFromLatest());
				DataStream<Point> spatialPointStream2 = Deserialization.TrajectoryStream(geoJSONStream2, inputFormat, inputDateFormat, inputDelimiter1, csvTsvSchemaAttr1, "timestamp", "oID", uGrid);
				DataStream<Tuple2<Point, Point>> collisions = new PointPointJoinQuery(realtimeConf, uGrid, uGrid).run(spatialPointStream1, spatialPointStream2, radius);

				collisions.print();
				break;
			}
			case 4: { //Speed
				DataStream geoJSONStream  = env.addSource(new FlinkKafkaConsumer<>(inputTopicName, new SimpleStringSchema(), kafkaProperties).setStartFromLatest());
				DataStream<Point> spatialPointStream = Deserialization.TrajectoryStream(geoJSONStream, inputFormat, inputDateFormat, inputDelimiter1, csvTsvSchemaAttr1, "timestamp", "oID", uGrid);

				DataStream<Tuple3<String, Double, Long>> velocityStream = spatialPointStream
						.keyBy(p -> p.objID)
						.process(new VelocityCalculator());

				velocityStream.print();


				break;
			}
			case 5: { //Point polygon range query

				DataStream geoJSONStream  = env.addSource(new FlinkKafkaConsumer<>(inputTopicName, new SimpleStringSchema()/*JSONKeyValueDeserializationSchema(false)*/, kafkaProperties).setStartFromLatest());

				DataStream<Point> spatialPointStream = Deserialization.TrajectoryStream(geoJSONStream, inputFormat, inputDateFormat, inputDelimiter1, csvTsvSchemaAttr1, "timestamp", "oID", uGrid);

				DataStream<Point> rNeighbors = new PointPolygonRangeQuery(realtimeConf, uGrid)
						.run(spatialPointStream, queryPolygonSet, 0.0001);

				rNeighbors.print();

				break;
			}
			case 6: { //PointPointKNNQuery - what robot is closest to a given point
				...
				DataStream<Point> spatialPointStream = spatialPointStream1
						.union(spatialPointStream2)
						.map(p -> {
							// Tworzymy nowy obiekt Point z aktualnym timestampem
							return new Point(
									p.objID,
									p.point.getX(),
									p.point.getY(),
									System.currentTimeMillis(), // nadpisujemy timestamp
									p.gridID
							);
						});
...
//TODO - przemyśleć łączenie streamów z rozbieżnymi timestampami
//			DataStream<Point> spatialPointStream = spatialPointStream1.union(spatialPointStream2)
//				.assignTimestampsAndWatermarks(
//						WatermarkStrategy.<Point>forBoundedOutOfOrderness(Duration.ofSeconds(1))
//								.withTimestampAssigner((point, ts) -> point.timeStampMillisec)
//				);

//				DataStream<Point> streamWithWatermarks = spatialPointStream1.union(spatialPointStream2)
//						.assignTimestampsAndWatermarks(
//								new BoundedOutOfOrdernessTimestampExtractor<Point>(Time.seconds(1)) {
//									@Override
//									public long extractTimestamp(Point element) {
...
//							public void process(String key, Context ctx, Iterable<Point> elements, Collector<Point> out) {
//								List<Point> sorted = StreamSupport.stream(elements.spliterator(), false)
//										.sorted(Comparator.comparingLong(p -> p.timeStampMillisec))
//										.collect(Collectors.toList());
//								sorted.forEach(out::collect);
//							}
//						});
//
//				streamWithWatermarks.print();

				Point p = new Point(queryPointCoordinates.get(0).x, queryPointCoordinates.get(0).y, uGrid);

				DataStream<Tuple3<Long, Long, PriorityQueue<Tuple2<Point, Double>>>> knnResults =
						new PointPointKNNQuery(realtimeConf, uGrid)
								.run(spatialPointStream, p,radius,1);
	...
	public static class VelocityCalculator extends KeyedProcessFunction<String, Point, Tuple3<String, Double, Long>> {

		private transient ValueState<Point> lastPoint;

		@Override
		public void open(Configuration parameters) {
			ValueStateDescriptor<Point> desc = new ValueStateDescriptor<>("lastPoint", Point.class);
			lastPoint = getRuntimeContext().getState(desc);
		}

		@Override
		public void processElement(Point current, Context ctx, Collector<Tuple3<String, Double, Long>> out) throws Exception {
			Point previous = lastPoint.value();
			if (previous != null) {
				double dx = current.point.getX() - previous.point.getX();
				double dy = current.point.getY() - previous.point.getY();
				double dt = (current.timeStampMillisec - previous.timeStampMillisec) / 1000.0;
				if (dt > 0) {
					double velocity = Math.sqrt(dx * dx + dy * dy) / dt;
					out.collect(new Tuple3<>(current.objID, velocity, current.timeStampMillisec));
				}
			}
